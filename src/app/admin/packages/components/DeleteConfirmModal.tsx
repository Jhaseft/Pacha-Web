...
export default function DeleteConfirmModal({ pkg, deleting, onConfirm, onCancel }: DeleteConfirmModalProps) {
  if (!pkg) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#111] border border-white/10 rounded-2xl w-full max-w-sm shadow-2xl p-6 flex flex-col items-center text-center gap-4">
        ...
        <div>
          <h3 className="text-white font-black text-lg">Eliminar paquete</h3>
          <p className="text-white/40 text-sm mt-1">
            ¿Estás seguro que quieres eliminar <span className="text-white/70 font-semibold">"{pkg.name}"</span>? Esta acción no se puede deshacer.
          </p>
        </div>
