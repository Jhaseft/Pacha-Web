...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="p-8 max-w-6xl">
      ...
      {!loading && hasPending && (
        <div className="mb-8">
          ...
          <div className="flex flex-col gap-2">
            ...
            <AlertBadge count={stats?.withdrawals.pending ?? 0} label="Retiros pendientes de anfitrionas" sub="Solicitudes de pago pendientes" />
          </div>
        </div>
      )}
      ...
      {/* ── Anfitrionas ── */}
      <div className="mb-8">
        <SectionLabel>Anfitrionas</SectionLabel>
        <div className="grid grid-cols-2 xl:grid-cols-3 gap-3">
          {loading ? Array.from({ length: 3 }).map((_, i) => <SkeletonCard key={i} />) : (<>
            <StatCard label="Total anfitrionas" value={stats?.anfitrionas.total ?? 0}
