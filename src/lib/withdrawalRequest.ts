...
export const apiGetWithdrawalRequests = async (search?: string, cursor?: string): Promise<WithdrawalRequestsResponse> => {
    try {
        ...
        const response = await apiAxios.get('/admin/anfitrionas/list/withdrawal-requests', { params });
...
export const apiCountPendingWithdrawalRequests = async (): Promise<number> => {
    try {
        const response = await apiAxios.get('/admin/anfitrionas/count/pending-withdrawal-requests');
